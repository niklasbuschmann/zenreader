...
const dropzone = state => <p style={`color: ${state.error ? '#f45' : (state.drag || state.success) ? '#0c6' : '#46f'}; border: 2px dashed; margin: 1.5em 0; padding: 1.5em; text-align: center; border-radius: 2px; width: 100%`}>{state.error ? state.error : state.success ? 'Successfully imported OPML' : 'Drop opml here to import'}</p>;

const Feeds = props =>
  <>
    <h2 className="center">Import / Export</h2>
