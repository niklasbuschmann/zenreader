...
const dropzone = state => <p style={{color: state.error ? '#f45' : (state.drag || state.success) ? '#0c6' : '#68f', border: '2px dashed', margin: '2em 0', padding: '1.5em', 'text-align': 'center', 'border-radius': '2px'}}>{state.error ? state.error : state.success ? 'Successfully imported OPML' : 'Drop opml here to import'}</p>;
